...
- **The count will rise as the last two groups are ported**, because their stubs predate the
  ...
  the internal reference's groupings: the `components/` pages bundling a sibling or a by-reference
  variant each split into several. The final number comes from enumerating each group against the source
  when it is written, not from the current tree.
