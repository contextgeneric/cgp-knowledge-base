...
- *An applied tutorial* — **new**. Building something real from an [example](../examples/README.md), for
  the reader who evaluates a technology by seeing a realistic system in it.

**Deep dives** (new category)
...
[writing-guides/deep-dive.md](writing-guides/deep-dive.md).

**Reference** (new category, ~70 pages)
- One page per construct, grouped as `macros/`, `attributes/`, `derives/`, `components/`, `providers/`,
  `traits/`, and `types/` — **new**, ported from the internal reference.
- A hand-written *index* page that names the handful of constructs a newcomer needs before listing the
  rest by job — **new**, and not an autogenerated list.
...
path is the Overview's problems → the *Extensible data types* deep dive → the
[examples](../examples/README.md). This is currently the least well-served reader, because that
material lives only in four blog posts whose code has drifted; the deep dive is what fixes it.
