...
- **The count will rise as the last three groups are ported**, because their stubs predate the
  ...
  the internal reference's groupings: the four provider catalogues, `use_field.md`, and the component
  pages bundling a sibling or a by-reference variant each split into several. The final number comes from
  enumerating each group against the source when it is written, not from the current tree.
