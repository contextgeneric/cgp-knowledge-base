...
- **The `types/` group is now ported**, split one page per type rather than the internal reference's
  ...
  [granularity rule](writing-guides/reference.md#granularity-one-page-per-named-construct): five
  top-level singletons (`phantom_data`, `field`, `index_type`, `life`, `mref`) and a `spines/`
  subdirectory (`cons`, `nil`, `either`, `void`, `chars`, `path_cons`) with its own overview, ordered by
  importance and adding a `PhantomData` page. Every type page that shows code has its `example-code`
  mirror.
