...
- **R1 — the reference index.** Hand-written, not autogenerated: it opens by naming the handful of
  ...
  scan, so this page is where the layered-audience promise is kept at the section level, and it is one of
  the surfaces the author reads. *Lands in:* `docs/reference/index.md`. *Blocked by:* nothing, and worth
  writing **first**, because it fixes the grouping every ported page then slots into.
- **R2 — port the construct pages.** Roughly seventy pages under `docs/reference/`, grouped as `macros/`,
  ...
  layered descent, add *When to reach for it*, and convert the Source section. One thing rides on this
  task beyond the pages themselves: the internal [guides](../cgp/guides/README.md) reach the public site
  **only** through the *When to reach for it* sections. This is the group that ships on its guide plus a
  spot check rather than a full read, and it is the natural unit for splitting the campaign into
  sessions — a subdirectory at a time. *Blocked by:* R1 for grouping, E2 and E3 for the concept links to
  point at, R4 for what a *Gotchas* section may defer to. *Done when:* every construct the `cgp` crate
  exports has a page or a named place inside a consolidated one.
...
- **R4 — the error catalog page.** A page inside the reference covering the compile errors CGP produces
  after codegen, so a reader who hits a wiring failure has somewhere on the site to look it up. The
  seventeen internal [error-class documents](../cgp/errors/README.md) are the material; the public page
  is one page rather than seventeen, organized by the catalog's own hidden-versus-surfaced axis, showing
  the small program behind each class and what `cargo cgp check` makes of it. Its existence is what lets
  every *Gotchas* section stay construct-specific instead of re-explaining the same failure, which is why
  it comes **before** the bulk of R2. *Lands in:* `docs/reference/`. *Blocked by:* nothing.
