...
The macro's parser is permissive about the body shape and surfaces most mistakes as generic `syn` parse errors rather than tailored diagnostics. An `open` header written after a plain mapping fails to parse because statements must lead the block, but by then the parser expects only mappings, so it reads `open` as a *key type* and reports `expected ':'` with the caret on the **component name** — blaming the component rather than the misplaced statement. A braced path group followed by more path reports the same `expected ':'` on the trailing segment, and a bounded generic list on a nested table reports `expected ','`, because the value parser tries the nested-table form speculatively and falls back to reading the whole value as a plain type. All three are pinned as rejection cases (see [Tests](#tests)).

**A nested table inside a `for` loop's body is emitted, and writing one well-formed takes care.** `DelegateEntries::extract_inner_tables` walks statements as well as mappings, so `for <T, Provider> in Table { @a.T: UseDelegate<new Inner<Provider> { … }> }` declares `Inner` and its impls. Two constraints meet on that value and only one shape satisfies both. The loop binds a provider variable the entry must mention, or the generated impl leaves it unconstrained (`E0207`); but the loop's variables are *not* in scope inside the lifted table's own impls, so mentioning it only there — `new Inner { X: Provider }` — fails on both counts at once, with `E0207` still on the outer impl and `E0425` on the `Provider` the inner impl cannot see. Declaring the variable as the inner table's own generic, `new Inner<Provider>`, is what puts it in both places.

The combination is nonetheless redundant in practice, which is why no wiring test drives one end to end: a loop already yields one provider per key, so a nested table below it dispatches the same parameter a second time. The form is kept correct rather than recommended.
...
- [namespaces/open_later_param.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-tests/tests/namespaces/open_later_param.rs) — `open` dispatch on the second type parameter alone, through keys whose first segment is an entry generic (`@DescriberComponent.<Code> Code.u64`); the snapshot pins the `PathCons<DescriberComponent, PathCons<Code, …>>` keys, and the test calls one context with two codes per input.
...
- [namespaces/redirect_mapping.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-tests/tests/namespaces/redirect_mapping.rs) — the `=>` operator written directly on a context: a component redirected to its own name, and a list key redirecting two components onto one shared `@`-path slot. Its second snapshot is the `open` spelling of the first entry, so the two goldens are identical modulo the context name — which is how the `open C;` ≡ `C => @C,` equivalence is pinned rather than asserted.
- [namespaces/combined_forms.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-tests/tests/namespaces/combined_forms.rs) — every body form in one block: an `open` statement, a `->` entry into an aggregate provider, a list key, and `@`-path keys using a braced group, a bracketed group, and a per-segment generic. The only place the composition itself is pinned.
...
- [namespaces/for_loop_nested_table.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-tests/tests/namespaces/for_loop_nested_table.rs) checks that a nested table opened inside a `for` loop's body is lifted out and carries its entry — the statement half of the extraction walk, which a mapping-only walk dropped. It asserts the lifted struct's `DelegateComponent` entry directly rather than wiring the component through, per the redundancy noted under [Known issues](#known-issues).

The failure cases in `cgp-macro-tests` pin the attribute rejection:

- [parser_rejections/delegate_components.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-macro-tests/tests/parser_rejections/delegate_components.rs) asserts the macro rejects an attribute on the table, on a key, and on a key nested inside a `UseDelegate<new Inner { … }>` value (the last confirms the validator recurses through mapping values rather than dropping the attribute), and that a braceless `open` header listing more than one component is rejected (the braceless form opens exactly one). Three further cases pin body-grammar rejections whose diagnostics are misleading enough to be worth guarding: a statement written after a mapping, a braced path group followed by more path (both reported as `expected ':'`), and a bounded generic list on a nested table (reported as `expected ','`, because the value parser falls back to reading the whole value as a plain type).
