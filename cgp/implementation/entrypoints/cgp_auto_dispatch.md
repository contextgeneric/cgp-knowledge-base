...
The generated method body **calls the matcher without naming the trait**, which fails in a module that imports `CanCompute`, whether by name or through a glob such as `cgp::extra::handler::*`. `derive_blanket_impl` emits `#matcher::<#computer_ident>::compute(&(), PhantomData::<()>, #args)` (or `compute_async`), a type-relative call that the compiler resolves against every trait in scope. The matcher implements the provider trait `Computer`, and the blanket `CanCompute` impl for every context also applies to it, so importing `CanCompute` beside the prelude's `Computer` makes the call ambiguous. A probe with `use cgp::extra::handler::CanCompute;` at the top of the module failed with `E0034` "multiple applicable items in scope" at each `#[cgp_auto_dispatch]` trait, the candidates being `Computer` for the matcher and `CanCompute` for `__Context__`. The fix is to emit the call fully qualified, as `<#matcher<#computer_ident> as Computer<(), (), #input_type>>::compute(…)`, using the same `#input_type` the `where` clause already names, and `AsyncComputer` for `compute_async`. The async body has the same form and would collide with `CanComputeAsync` the same way, though the probe tested only the synchronous case. The user-visible consequence is under Known issues in [reference/macros/cgp_auto_dispatch.md](../../reference/macros/cgp_auto_dispatch.md).
