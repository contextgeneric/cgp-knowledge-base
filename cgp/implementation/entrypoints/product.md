...
- [extensible_records/product_ops.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-tests/tests/extensible_records/product_ops.rs) pins the type-level operations over the spine this macro builds — [`AppendProduct`, `ConcatProduct`, and `MapFields`](../../reference/traits/product_ops.md) — as type equalities, including both `Nil` identity cases and `MapFields` over the `Either`/`Void` sum spine as well as the product one. All three are public traits and none had a test before.
