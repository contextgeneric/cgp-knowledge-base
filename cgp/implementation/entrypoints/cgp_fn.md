...
Three further `#[impl_generics]` failures are deferred to the compiler the same way, each a plain error on the emitted impl. A parameter absent from every implicit argument's type is unconstrained (`E0207`, caret on the parameter inside the attribute), because the field bound is the only thing that can determine it. A parameter named after the function collides with the generated trait, whose name is the function's in PascalCase, so inside the impl the trait's name resolves to the parameter (`E0404`, caret on the function name). And a parameter default such as `T = u32` parses, because the argument is a `syn::GenericParam`, and the compiler rejects it on the impl with the deny-by-default `invalid_type_param_default` lint. None of these is caught by the macro, and each is recorded in [reference/attributes/impl_generics.md](../../reference/attributes/impl_generics.md).
...
- [implicit_arguments/cgp_fn_calling_fn.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-tests/tests/implicit_arguments/cgp_fn_calling_fn.rs) — one `#[cgp_fn]` trait depending on another through an explicit `where Self:` bound.
...
- [implicit_arguments/cgp_fn_calling_fn.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-tests/tests/implicit_arguments/cgp_fn_calling_fn.rs) confirms one trait's method calling another resolves through both blanket impls at run time.
