...
- **parse** reads the header and the entry table. The entry table is the same `DelegateEntries` type that `delegate_components!` parses, so a namespace body accepts the same mappings, array keys, `open`/`namespace`/`for` statements, and `=>` redirects. The table is optional: when nothing follows the header, the entries default to the empty table.
...
The body **reuses the `delegate_components!` grammar wholesale** — array keys, `@`-path keys with their bracketed and braced groups, the leading `open`/`namespace`/`for` statements, and all three of the `:`, `->`, and `=>` mappings parse, because the entries are a `DelegateEntries`. Anything `delegate_components!` accepts in a table, a namespace body accepts too, lowered to `impl Namespace<__Table__> for Key` rather than to a `DelegateComponent` impl.

**A nested `Wrapper<new Inner { … }>` value is lifted out here exactly as it is in a delegation table.** `NamespaceTable::eval` runs the same `ExtractInnerDelegateTables` walk `DelegateTable::eval` does, emitting each inner table's `EmptyStruct` into `inner_structs` and its `DelegateComponent`/`IsProviderFor` impls onto `item_impls`. The entry's own `Delegate` is the wrapper over the generated table type, so the namespace holds the dispatch table and every context joining it inherits the per-type wiring without restating it. The walk is not optional: the two macros share `DelegateEntries`, so a namespace body accepts the value whether or not the evaluation emits its table, and skipping it leaves the entry naming a struct nothing declares — an `E0425` on the inner table's name, which reads like a typo rather than a dropped table.

**A header-only namespace omits its braces.** `NamespaceTable::parse` checks whether the input is exhausted after the optional `: parent` clause and, if so, takes `DelegateEntries::default()` as the body instead of demanding a brace pair. `new Child: Parent` therefore emits the struct, the trait, and the inheritance impl, identical to `new Child: Parent { }`, and `new Ns` alone emits just the struct and the trait. The check runs only on exhausted input, so a token that is not a `{` fails inside `braced!` with `expected curly braces`. The optional body is specific to the namespace header; `delegate_components!`, the nested-table value, and the `for` loop keep their braces even when the body is empty.
...
- [namespaces/namespace_header_only.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-tests/tests/namespaces/namespace_header_only.rs) — the header-only form written without a brace pair: `new HeaderOnlyNamespace` emitting just the struct and trait, and `new HeaderOnlyExtendedNamespace: DefaultNamespace` emitting the struct, trait, and inheritance impl, byte-for-byte what the braced empty bodies produce.
- [namespaces/namespace_nested_table.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-tests/tests/namespaces/namespace_nested_table.rs) — a legacy `UseDelegate<new FooTable { … }>` value in a namespace body, pinning the lifted `FooTable` struct and its two `DelegateComponent` pairs rendered after the trait and before the namespace impl.
...
- [namespaces/namespace_nested_table.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-tests/tests/namespaces/namespace_nested_table.rs) checks that a nested table declared in a namespace body resolves for two contexts that join it, which is what putting the dispatch table in the namespace rather than on a context buys.
- [namespaces/namespace_header_only.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-tests/tests/namespaces/namespace_header_only.rs) joins a context to the header-only `HeaderOnlyExtendedNamespace: DefaultNamespace` and wires the inherited error components under their full `@cgp.core.error.*` paths, confirming a braceless definition is a usable namespace rather than only a successful parse.
...
- [parser_rejections/cgp_namespace.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-macro-tests/tests/parser_rejections/cgp_namespace.rs) asserts the macro rejects an attribute on a `:` mapping key, on a `=>` redirect key, and on a key inside a `for` loop. The same file asserts that a token other than `{` after the header (`new MyNamespace: DefaultNamespace;`, `new MyNamespace foo`) is rejected, so the header-only form accepts only an input that ends at the header.
