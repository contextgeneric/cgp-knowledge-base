...
**Generic parameters** on the component are appended after the context in the provider trait and collected into the `IsProviderFor` params tuple. A lifetime stays ahead of the context (lifetimes must precede type parameters) and is lifted into `Life<'a>` in that tuple, so `HasReference<'a, T>` yields `IsProviderFor<…, (Life<'a>, T)>`. Only *type* parameters, not lifetimes or const parameters, extend the `RedirectLookup` lookup path, and `generic_params_to_path` in `to_redirect_lookup_impl` appends *all* of them in declaration order, as one `ConcatPath` bound over the whole list. So `CanCompute<Code, Input>` looks up `…Code.Input`, and a table entry may key on any prefix of that list; this is what lets `open` dispatch on a later parameter without a second `#[derive_delegate]` dispatcher.
...
- [error_backends/eyre_location.rs](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-tests/tests/error_backends/eyre_location.rs) checks that a method attribute reaches every generated impl: `CanRaiseError::raise_error` is `#[track_caller]`, the macro copies the attribute onto the provider trait's declaration, and an eyre report raised through plain, `open`, and namespace-path wiring records the test's own line as its location.
