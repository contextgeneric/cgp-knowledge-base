...
Two registrations for one key in one table conflict. Each emits an impl of the lookup trait for the same key, and the compiler rejects the second with `E0119` (`conflicting implementations of trait 'DefaultImpls1<ShowImplComponent, _>' for type 'String'`). Both carets land on the key tokens inside the two attributes, because the impl is re-spanned onto its key. The same conflict arises between a `#[default_impl(GreeterComponent in DefaultNamespace)]` and a `#[prefix(… in DefaultNamespace)]` on the same component, since both emit an impl of `DefaultNamespace` for the marker; `cargo-cgp` reports that one as `[CGP-E007]`, naming the `@app.GreeterComponent` path the registration should have used, since a prefixed component is bound at its path rather than at its marker.

A prefixed component must be registered by its **path**, not its bare marker, and writing the marker collides for the same reason one step removed. `#[prefix(@app in DefaultNamespace)]` makes `DefaultNamespace` answer `GreeterComponent` with a redirect, so a namespace that inherits `DefaultNamespace` already covers that key through its inheritance blanket, and `#[default_impl(GreeterComponent in AppNamespace)]` overlaps it under `E0119` — where `#[default_impl(@app.GreeterComponent in AppNamespace)]` is what binds the provider the redirect leads to. This is the [namespace override conflict](../../errors/wiring/namespace-override-conflict.md) class, which `cargo-cgp` likewise reports as `[CGP-E007]` naming the path to write.

A registered default is final for every context that draws on it; a context adds entries beside the defaults, never over them. A context that joins a namespace cannot replace a path the namespace itself binds through a `#[default_impl]`, because the join's blanket impl already covers that path and a direct entry collides with it under `E0119`. Per-type defaults behave the same way: a context that pulls a `DefaultImpls1` registry in with a `for` loop can add a direct entry only for a type the registry does not cover, since for a covered type the loop's impl and the entry overlap under the same `E0119`. This is the [namespace override conflict](../../errors/wiring/namespace-override-conflict.md) error class, and the reason the [guide](../../guides/namespaces-and-prefixes.md) registers prefixes and defaults into different namespaces.
