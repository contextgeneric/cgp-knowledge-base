...
The public reference is organized one page per named construct, so this document feeds **4 pages** rather than one: [`map_type`](https://contextgeneric.dev/docs/reference/traits/type-level/map_type), [`map_type_ref`](https://contextgeneric.dev/docs/reference/traits/type-level/map_type_ref), [`transform_map`](https://contextgeneric.dev/docs/reference/traits/type-level/transform_map), [`transform_map_fields`](https://contextgeneric.dev/docs/reference/traits/type-level/transform_map_fields). A change here is propagated to each of them, per the [synchronization rule](../../../AGENTS.md#the-synchronization-rule); the mapping and the granularity rule behind it are recorded in [website/site-structure.md](../../../website/site-structure.md).
