...
**The imported bound need not be a CGP capability.** An ordinary Rust trait the provider requires of its context is imported exactly the same way — `#[uses(Display)]` and `#[uses(AsRef<[u8]>)]` are preferred over `where Self: Display` and `where Self: AsRef<[u8]>` — because the attribute declares a dependency on `Self` regardless of where the trait came from, and using it for a familiar trait shows that it is not CGP-specific machinery. Reserve the hand-written `where` clause for the bounds `#[uses]` genuinely cannot carry, listed below and in the [guides summary](README.md#summary).
