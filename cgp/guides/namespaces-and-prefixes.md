...
The same prefix-or-not split decides how the key is *written*, and getting that wrong is the easiest mistake to make with the attribute. A prefixed component is addressed by its path, so its default registers as `#[default_impl(@app.GreeterComponent in AppNamespace)]`; writing the bare `#[default_impl(GreeterComponent in AppNamespace)]` claims a key the namespace already answers through the prefix's redirect — directly when it is the same namespace the `#[prefix]` names, through the inheritance blanket when the namespace inherits it — and the compiler rejects the overlap with `E0119`. Only an un-prefixed component is registered by its marker. `cargo-cgp` reports this one as `[CGP-E007]` and names the path to use in a `help`, and the [namespace override conflict](../errors/wiring/namespace-override-conflict.md) class has the full anatomy.
