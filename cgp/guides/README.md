...
- [Choosing a component's shape](choosing-a-component-shape.md) — what goes in the `Self` position, and whether the component targets `Self` or a type parameter; the decision that comes before every other one here, since it cannot be revised without a breaking change.
- [Sizing a component](sizing-a-component.md) — which items belong in one component: the ones a single provider choice decides together, which is often one method and sometimes several, with the three costs of grouping unrelated decisions and how to split a trait that has.
...
- [Naming a type dependency](naming-a-type-dependency.md) — where a type the implementation needs should live: inferred from a field with `#[impl_generics]`, or declared as an abstract type the context supplies — and why a generic parameter on the trait is the form both exist to avoid.
...
- [Adding method supertraits](method-supertraits.md) — `#[extend]` instead of native `:` supertrait syntax.
...
|---|---|---|
| define a component ([guide](choosing-a-component-shape.md)) | an operation about the application, self-targeted on an environmental context — and a `Value` parameter only when the target type is one you do not own | inheriting whichever shape the last example you read used |
| decide which items go in one component ([guide](sizing-a-component.md)) | the items one provider choice decides together — a method, plus an associated type only that provider fixes | grouping decisions a context would want to make separately, as an entity trait does |
...
| require a trait on the context or an inner provider ([guide](declaring-dependencies.md)) | `#[uses(Trait)]` / `#[use_provider(P: Trait)]`, comma-separated in one attribute — `#[uses]` covers ordinary Rust traits too (`#[uses(AsRef<[u8]>)]`) | hand-written `Self:`/`P: Trait<Self>` `where` bounds |
...
| give a type the implementation needs a home ([guide](naming-a-type-dependency.md)) | `#[impl_generics]` while the type only flows through implicit arguments; an abstract type once it appears in a signature or two such types must agree | a generic parameter on the trait, which every caller and every intermediate has to declare and bound |
...
| add a non-type supertrait that contributes methods ([guide](method-supertraits.md)) | `#[extend(Trait)]` | native `: Supertrait` inheritance syntax |
