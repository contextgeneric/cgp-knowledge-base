...
One adjacent code is worth knowing, because a grep for `E0433` will not find it. A name left unresolved inside a macro attribute's *own* tokens is reported as [`E0425`](../error_codes/e0425.md) instead, with the same "cannot find type" headline but the bare label `not found in this scope`. That is the shape of the [cyclic-context sibling](unresolved-imported-type.md#a-sibling-the-unresolved-context), where the caret sits inside the `#[use_type(...)]` attribute rather than in the code the attribute rewrote.
...
- [Unresolved imported abstract type](unresolved-imported-type.md) — the sibling lowering class, where a `#[use_type]` import names a type the trait does not declare (`E0576`), or an `in Context` cycle leaves an alias ungrounded (`E0425`).
