...
## A second sibling: the reserved variant name

The third shape in this family is the only one that *is* a CGP defect, and the only one whose readability depends on which construct produced it. The extensible-data derives name their own associated types through an unqualified `Self::…` path — `Self::Value` in [`#[derive(FromVariant)]`](../../reference/derives/derive_from_variant.md)'s constructor, `Self::Value` and `Self::Remainder` in [`#[derive(ExtractField)]`](../../reference/derives/derive_extract_field.md)'s extract impls, `Self::Extractor`, `Self::ExtractorRef`, and `Self::ExtractorMut` in its accessors, and `Self::Fields` and `Self::FieldsRef` in [`#[derive(HasFields)]`](../../reference/derives/derive_has_fields.md)'s conversions. Inside an impl for an *enum*, each of those paths can resolve either to the trait's associated type or to a variant of the same name, so seven variant names cannot be used:

```rust
#[derive(FromVariant)]
pub enum Tagged {
    Value(u32), // `Self::Value` in the generated constructor is now ambiguous
}
```

The compiler reports a bare `error: ambiguous associated item` — no `E` code — headlined on the `#[derive(...)]` attribute, with two notes naming the two candidates: "`Value` could refer to the variant defined here" and "`Value` could also refer to the associated type defined here".

**Whether that first note is useful depends on which derive collided, and the difference is the reason this shape is worth cataloging.** `#[derive(HasFields)]` and `#[derive(FromVariant)]` write their impls `for` the user's own enum, so the colliding variant keeps its own span and the note points straight at it — an ordinary, readable ambiguity error. `#[derive(ExtractField)]` writes its impls for the generated `__Partial…` companion enums instead, and the codegen rebuilds those variant identifiers rather than reusing the user's, so they carry the derive's span: *both* notes then land on the `#[derive(ExtractField)]` attribute and nothing in the output names the variant the author must rename. That is the case a reader cannot resolve from the diagnostic alone, and the one to recognize by shape.

Unlike the shadowed abstract type above, this is a defect rather than ordinary name resolution. The codegen should write each associated type as a fully qualified projection — `<Self as FromVariant<Tag>>::Value` — which is unambiguous whatever the variants are named. Until it does, the fix is to rename the variant, and the seven reserved names are recorded in the Known issues of each affected derive's [reference](../../reference/derives/derive_extract_field.md) and [implementation](../../implementation/entrypoints/derive_extract_field.md) document. A struct's *field* names are unaffected, since a field is not in the same namespace as an associated type.
...
- The [reserved variant name](#a-second-sibling-the-reserved-variant-name) has **no `cargo-cgp` fixture yet**, which is a recorded gap rather than an oversight: the shape is pinned on the library side instead, by [`invalid_expansion/reserved_variant_names.rs`](https://github.com/contextgeneric/cgp/blob/main/crates/tests/cgp-macro-tests/tests/invalid_expansion/reserved_variant_names.rs), which captures the emitted `Self::…` paths as a string snapshot so the test compiles even though the code it describes would not. What that pin does not cover is the rendered diagnostic — whether the tool leaves this message as the compiler wrote it, and whether it could recover the variant name the compiler omits. A fixture is what would settle both.
