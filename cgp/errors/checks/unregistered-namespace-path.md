...
The shape scales with how the redirect fails. A deep prefix renders its whole path (`@app.finance.types.QuantityTypeProviderComponent`); a chain of redirects that never terminates prints one `[CGP-E104]` hop per link before the `[CGP-E107]` leaf (`@start.GreeterComponent` → `@middle` → `@end`); and an `open` per-type dispatch missing a key names the type in the path (`@ItemEncoderComponent.Vec<u8>`). When the context wires nothing at the path, the leaf is `[CGP-E107]`. When the path is missing from an aggregate provider, whether it `open`s the component in its own table or joins the namespace itself, the leaf is the `[CGP-E110]` provider-table leaf instead, `` provider `ByteSink` does not contain any delegate entry for `@ComputerComponent.Sink.Digest` ``, and the `[CGP-E104]` hop above it reads `` in `ByteSink` ``, so the tree names the table the entry belongs in. The codes are defined in the [cargo-cgp error-code catalog](../../../cargo-cgp/error-code.md).
...
- [`acceptable/wiring/missing-wiring/open_aggregate_missing_entry.rs`](https://github.com/contextgeneric/cargo-cgp/blob/main/tests/ui/acceptable/wiring/missing-wiring/open_aggregate_missing_entry.rs) — a context routes one code to an aggregate provider that `open`s `ComputerComponent` in its own table and dispatches on the input, and the aggregate lacks the input the check asks for; the `.cgp.stderr` pins the `[CGP-E110]` leaf naming the aggregate and the `[CGP-E104]` hop that reads `` in `ByteSink` ``.
- [`acceptable/wiring/missing-wiring/namespace_aggregate_missing_entry.rs`](https://github.com/contextgeneric/cargo-cgp/blob/main/tests/ui/acceptable/wiring/missing-wiring/namespace_aggregate_missing_entry.rs) — a context forwards `@app` to a path-keyed aggregate provider that joins `DefaultNamespace` itself and lacks `@app.NamerComponent`; the `.cgp.stderr` pins the `[CGP-E110]` leaf naming the aggregate, whose namespace lookup is the unmet bound.
