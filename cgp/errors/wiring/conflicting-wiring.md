...
// Overlapping path — one `@`-path key is a prefix of another (E0119).
delegate_components! {
    App {
...
Two different path keys conflict when one covers the other. Every `@`-path key lowers to a `DelegateComponent<PathCons<…, __Wildcard__>>` impl whose tail is a free parameter, so a key covers every longer key beneath it: `@cgp.core.error.ErrorTypeProviderComponent` already claims `@cgp.core.error.ErrorTypeProviderComponent.String`. A per-entry generic segment matches every value in its position, so `@ComputerComponent.<Code> Code.u64` overlaps `@ComputerComponent.Eval` too, at `Code = Eval`. Both forms are easy to write while converting nested `UseDelegate` and `UseInputDelegate` tables to the [`open` statement](../../reference/macros/delegate_components.md#statements-open-namespace-and-for), by keying one code on its own beside keys for the same code, or for every code, per input.
...
An overlapping path key prints the same `E0119` pair as a duplicated one, including the "downstream crates may implement" note on the `IsProviderFor` half, and only the carets tell them apart. For a duplicate both carets underline the same path. For an overlap the "first implementation here" caret sits on the shorter key and the conflicting one on the longer, and the conflicting trait is named with the longer key, its wildcard tail printed as `_`: `DelegateComponent<PathCons<…, PathCons<ErrorTypeProviderComponent, PathCons<String, _>>>>`.
...
- A path key overlapping a longer one becomes **`[CGP-E005]` overlapping wiring**, naming the longer key as the one that cannot be wired and the shorter as what already covers it: `` [CGP-E005] `App` cannot wire `@cgp.core.error.ErrorTypeProviderComponent.String.*` that is already set through `@cgp.core.error.ErrorTypeProviderComponent.*` ``. The trailing `.*` on both is the wildcard tail, and it is what shows that the second path lies beneath the first. A generic segment renders as `*` in its own position, so `@ComputerComponent.<Code> Code.u64` beside `@ComputerComponent.Eval` reads `` `App` cannot wire `@ComputerComponent.*.u64.*` that is already set through `@ComputerComponent.Eval.*` ``.
...
Remove one of the two entries. For an overlapping path key, keep either the shorter key or the longer ones: key the first segment on its own when one provider serves every later value, or key each later value when they differ, since one table cannot do both. A table that dispatches on a later parameter through a generic first segment therefore cannot also key any first-parameter value on its own. For a duplicate check-trait name from two tables over one context, add a `#[check_trait(Name)]` to one. The namespace collisions split off above have their own remedies — inheriting rather than joining several namespaces, or overriding only a path the namespace leaves unbound — covered in [overlapping namespace forwarding](namespace-forwarding-conflict.md) and [namespace override conflict](namespace-override-conflict.md).
...
- [`wiring/namespace-paths/path_sub_key_overlap.rs`](https://github.com/contextgeneric/cargo-cgp/blob/main/tests/ui/acceptable/wiring/namespace-paths/path_sub_key_overlap.rs) — `@cgp.core.error.ErrorTypeProviderComponent` beside `@cgp.core.error.ErrorTypeProviderComponent.String`, the prefix overlap; the `.rust.stderr` pins the `E0119` pair with carets on the two keys, the `.cgp.stderr` the `[CGP-E005]` headline naming both paths.
- [`wiring/namespace-paths/path_generic_segment_overlap.rs`](https://github.com/contextgeneric/cargo-cgp/blob/main/tests/ui/acceptable/wiring/namespace-paths/path_generic_segment_overlap.rs) — `@ComputerComponent.<Code> Code.u64` beside `@ComputerComponent.Eval`, the overlap through a generic first segment; the `.cgp.stderr` pins the `[CGP-E005]` headline with the generic segment rendered as `*`, `@ComputerComponent.*.u64.*`.
...
The two namespace collisions that are *not* duplicate declarations live with their own classes: `override_registered_path.rs`, `inherited_override_conflict.rs`, and `for_loop_prefixed_key.rs` under [namespace override conflict](namespace-override-conflict.md), and `for_loop_bare_key.rs` and `two_namespaces_joined.rs` under [overlapping namespace forwarding](namespace-forwarding-conflict.md).
