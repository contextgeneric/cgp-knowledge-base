...
There is a second reason, independent of swappability and easy to miss because vanilla Rust offers no comfortable alternative: an abstract type is **not threaded**. The other way to leave a type open is a generic parameter, and a parameter is an *input* the caller supplies, so it lands in the signature and every intermediate function that merely passes a value along has to declare it and repeat its bounds. An abstract type is an *output* the context determines, so it is named without being chosen at any call site. A pipeline of three operations over a scalar, an error type, and a runtime is `<Scalar, E, R>` plus their bounds at every layer when they are parameters, and nothing at all when they are abstract types — the layers that do not touch a scalar never mention one. This is why abstract types are what let generic code accumulate type dependencies without its signatures growing: the count of things a context decides can rise freely, because deciding is not passing. The mechanism is [impl-side dependencies](impl-side-dependencies.md#type-dependencies-and-why-they-need-no-parameter) applied to types, and the choice between an abstract type and a parameter is worked out in [naming a type dependency](../guides/naming-a-type-dependency.md).
