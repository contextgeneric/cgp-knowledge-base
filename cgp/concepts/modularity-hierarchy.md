...
This document is descriptive; the prescriptive companion is [choosing a component's shape](../guides/choosing-a-component-shape.md), which carries the default to reach for, the cost of each alternative, the refactoring that promotes a self-targeted component to a parameter-targeted one, and the two traps — that a parameter is not always a target, and that per-application choice needs no parameter at all. A rung says nothing about how large a component should be, and that decision cuts across all of them: [sizing a component](../guides/sizing-a-component.md) covers why a component grouping several capabilities collects less reuse at every rung, and why a trait whose providers no second context could reuse whole belongs back at rung 2.
