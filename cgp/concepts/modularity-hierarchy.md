...
Adopting CGP is not an all-or-nothing jump from ordinary Rust traits to fully context-generic code. The same operation, say serializing a value, can be expressed at several levels of modularity, and the levels form a gradient from "exactly one implementation, no wiring" to "any number of implementations, wired per type per provider." Each higher tier admits more overlapping or orphan implementations that vanilla Rust's [coherence](coherence.md) rules would reject, and each one costs something: more boilerplate, a changed interface, or tighter coupling between providers. Reading the tiers in order shows what each technique buys and what it asks for, so a use case can settle at the lowest tier that still works rather than reaching for the most powerful tool by default.

The hierarchy is illustrated throughout with one running operation, serializing a value, mirroring the [modular serialization](../../examples/modular-serialization.md) example, so the only thing that changes between tiers is the modularity technique rather than the problem. All snippets assume `use cgp::prelude::*;`.
...
A value context with a target parameter is legal and uninteresting, since the operation would act on one type while being wired on another for no gain.
...
The guiding rule is to settle at the lowest tier that expresses the use case, because each higher tier trades simplicity for modularity that may not be needed. An operation with one universal implementation belongs on tier 1; one that varies by type but never by application belongs on tier 2; a trait that should gain alternative providers without changing its interface belongs on tier 3 in its retrofit shape; an operation *about the application itself* belongs on tier 3 in its application shape, which is where most CGP code lives; an operation where different applications must treat the same foreign type differently belongs on tier 4; and only a provider that must override a nested type's wiring locally needs tier 5. Reaching for a higher tier than necessary adds context parameters, wiring, and coupling that buy nothing, while stopping too low forces the hand-written impls and global commitments the higher tiers exist to avoid.
